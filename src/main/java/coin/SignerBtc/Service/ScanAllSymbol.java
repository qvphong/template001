...
public class ScanAllSymbol {
  ...
  public List<BinanceCandlestick> getOldCandltick(BinanceSymbol symbol) throws BinanceApiException {
    ...
    List<BinanceCandlestick> kLine = api.klines(symbol, BinanceInterval.FIFTEEN_MIN, 7, null);
    if (kLine.isEmpty() || kLine.size() < 3) {
      logger.info("Symbol: "+ symbol + " data size: " + kLine.size());
      return null;
    }
